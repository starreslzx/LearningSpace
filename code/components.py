...
class CategoryCard(BoxLayout):
    """分类卡片组件 - 使用 BoxLayout 而不是 FloatLayout 避免图形问题"""
    ...
    def __init__(self, **kwargs):
        ...
        self.background_color = [0.95, 0.95, 0.95, 1]  # 浅灰色背景
...
class QuestionCard(BoxLayout):
    ...
    def create_content(self):
        ...
        view_btn = Button(
            text="查看",
            font_size='12sp',
            background_color=(0.4, 0.7, 0.4, 1),  # 绿色
            color=(1, 1, 1, 1)  # 白色文字
        )
        ...
        edit_btn = Button(
            text="编辑",
            font_size='12sp',
            background_color=(0.3, 0.5, 0.8, 1),  # 蓝色
            color=(1, 1, 1, 1)  # 白色文字
        )
...
class PathBreadcrumb(BoxLayout):
    ...
    navigate_callback = ObjectProperty(None)  # 确保属性名匹配
    ...
    def update_breadcrumb(self, instance, value):
        ...
        for i, item in enumerate(self.path_items):
            ...
            path_btn = Button(
                ...
                size_hint_x=None,
                width=min(len(item['name']) * 10 + 20, 150),  # 限制最大宽度
                background_normal='',
                background_down=''
            )
            ...
            if i < len(self.path_items) - 1:
                ...
                path_btn.disabled = True  # 当前项禁用点击

            self.add_widget(path_btn)

    def on_item_click(self, item_id):
        """点击路径项"""
        print(f"PathBreadcrumb: 点击了分类ID: {item_id}")
...
class AutoHeightLabel(Label):
    ...
    def __init__(self, **kwargs):
        ...
        self.bind(
            text=self.update_height,
            width=self.update_height,
...
class UploadPopup(Popup):
    ...
    def __init__(self, upload_callback=None, **kwargs):
        ...
        self.title = "上传题目材料"
        self.size_hint = (0.8, 0.7)  # 增加高度以容纳更多按钮
...
class QuestionPreviewPopup(Popup):
    ...
    def create_ui(self):
        ...
        self.type_input = TextInput(
            text=self.question_data.get('type', '选择题'),
            multiline=False,
            size_hint_x=0.7
        )
        ...
        self.category_input = TextInput(
            text=self.question_data.get('category', '默认'),
            multiline=False,
            size_hint_x=0.7
        )
        ...
        self.answer_input = TextInput(
            text=self.question_data.get('answer', ''),
            multiline=True,
            size_hint_y=0.8
        )
